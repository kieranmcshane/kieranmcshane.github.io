...
\title{Ecole Polytechnique--ESPCI 2026\\Mathematics B\\V2 Concours-Grade Correction}
...
From the formula of Question 5c, expanded formally with $D^2=4-X^2$, one obtains
\[
\begin{aligned}
  \chi_n(X)
...
\medskip
\noindent\emph{Scaling note.}  The proof above uses the scaled polynomials
$P_m((X-c_i)/\rho_i)$.  This is the same construction as the indicated one, but it keeps the argument of $P_m$ inside $[-1,1]$, where Question 10b applies.  Since $\rho_i>0$, the step function is unchanged:
...
Fix $k\ge0$, $B>4$, and $\varepsilon>0$.  Let $\ell\ge k$.  Since $|x|>B>1$ implies $|x|^k\le |x|^\ell$, we have
\[
  g_{k,B}\le g_{\ell,B}.
\]
Therefore
\[
  \Prob(S_n(g_{k,B})\ge\varepsilon)
  \le \Prob(S_n(g_{\ell,B})\ge\varepsilon).
\]
By Question 14,
\[
  \Prob(S_n(g_{\ell,B})\ge\varepsilon)
  \le \frac{\E(S_n(f_{2\ell}))}{\varepsilon B^\ell}.
\]
Using Question 13a and hypothesis $(H_{2\ell})$,
\[
  \E(S_n(f_{2\ell}))\longrightarrow\Sigma(f_{2\ell}).
\]
By Question 13b,
\[
...
\[
  ...
  =\frac1\varepsilon\left(\frac4B\right)^\ell.
\]
Now let $\ell\to\infty$.  Since $B>4$, the right-hand side tends to $0$.  Hence
\[
...
\[
  |S_n(f_j)-\E(S_n(f_j))|\ge\frac{\varepsilon}{4(d+1)\max(1,C_1)}
  \qquad(0\le j\le d),
\]
...
\begin{itemize}[leftmargin=2em]
  ...
  now supplied by \texttt{WignerInputConcrete.lowDegreeMomentAPI}.  The last
