...
Lorsqu'un objet est introduit ou identifié, sa nature mathématique est nommée explicitement : « le polynôme $R$ défini par \dots », « la suite $(u_n)$ définie par \dots », « l'application $f\colon E\to F$ », etc. La notation ne doit pas porter seule cette information de catégorie ; la phrase doit permettre au lecteur de reconnaître immédiatement le type de l'objet.

Les transformations successives d'une même expression sont disposées sur plusieurs lignes, avec les signes d'égalité alignés :
\[
\begin{aligned}
A&=B\\
 &=C\\
...
\end{aligned}
\]
Cette présentation rend chaque étape identifiable et doit être préférée aux longues chaînes d'égalités composées sur une seule ligne.
...
\begin{enumerate}
  ...
  \item
  \begin{align*}
  \overline{1+2\ii}+\overline{-4+6\ii}
  &=(1-2\ii)+(-4-6\ii)\\
  &=\boxed{-3-8\ii}.
  \end{align*}
  \item
...
\begin{enumerate}
  ...
  \item On développe directement le produit demandé :
  \begin{align*}
  (z-1)Q(z)
  &=z(z^4+z^3+z^2+z+1)-(z^4+z^3+z^2+z+1)\\
  &=(z^5+z^4+z^3+z^2+z)-(z^4+z^3+z^2+z+1)\\
  &=z^5-1=P(z).
  \end{align*}
  Les termes de degrés $1$ à $4$ s'annulent deux à deux. Ainsi, toute racine de $Q$ est une racine de $P$. Réciproquement, si $P(z)=0$ et $z\neq1$, alors l'égalité $(z-1)Q(z)=0$ impose $Q(z)=0$. Comme $Q(1)=5\neq0$, les racines de $Q$ sont exactement les racines cinquièmes de l'unité autres que $1$, soit
  \[
  ...
  y^2+y-1=0.
  \]
  Ainsi $y$ est racine du polynôme $R(Y)=Y^2+Y-1$.

  \item Les racines du polynôme $R$ sont
  \[
...
\begin{enumerate}
  ...
  \item Écrivons d'abord les deux différences :
  \begin{align*}
  z_Q-z_P
  &=\frac{z_C-z_A}{2}
    -\frac{\sqrt3}{6}\ii\,(z_A-2z_B+z_C),\\
  z_R-z_P
  &=\frac{z_C-z_B}{2}
    -\frac{\sqrt3}{6}\ii\,(2z_A-z_B-z_C).
  \end{align*}
  En multipliant la première par $\e^{\ii\pi/3}
  =\frac12+\frac{\sqrt3}{2}\ii$, puis en regroupant les coefficients
  de $z_A,z_B,z_C$, on obtient
  \begin{align*}
  \e^{\ii\pi/3}(z_Q-z_P)
  &=\left(\frac12+\frac{\sqrt3}{2}\ii\right)
    \left[
      \frac{z_C-z_A}{2}
      -\frac{\sqrt3}{6}\ii\,(z_A-2z_B+z_C)
    \right]\\
  ...
  &=z_R-z_P.
  \end{align*}
  Ainsi $\overrightarrow{PR}$ est l'image de $\overrightarrow{PQ}$ par une rotation de $\pi/3$. Les deux vecteurs ont même norme et forment un angle de $\pi/3$ ; le triangle $PQR$ est donc équilatéral.
\end{enumerate}
...
\end{methode}
\begin{enumerate}
  \item Écrivons $I_k=[a_k,b_k]$ pour $k\in\{1,2,3\}$, puis posons
  \[
  \alpha=\max(a_1,a_2,a_3),
  \qquad
  \beta=\min(b_1,b_2,b_3).
  \]
  Choisissons $i_0,j_0\in\{1,2,3\}$ tels que
  $\alpha=a_{i_0}$ et $\beta=b_{j_0}$. Puisque
  $I_{i_0}\cap I_{j_0}\neq\varnothing$, on a
  $a_{i_0}\leq b_{j_0}$, donc $\alpha\leq\beta$. Par conséquent,
  \[
  I_1\cap I_2\cap I_3=[\alpha,\beta]\neq\varnothing.
  \]
  Le dessin permet d'anticiper ce résultat, mais l'argument ci-dessus le démontre.
