  ...
  <section class="rating-lab-protocol" id="protocol" aria-labelledby="protocol-heading">
    ...
    <details class="rating-lab-limitations">
      ...
      <ul>
        ...
        <li>Rating-history event ticks are descriptive annotations, not model inputs. The checked-in sport-specific rule set selects major competitions and retains at most the latest 16 per competitor; each tick uses the first covered result date and publishes the competitor’s covered W–D–L record. It does not infer an official finishing round when the source lacks round metadata.</li>
