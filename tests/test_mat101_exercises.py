...
class Mat101ExerciseLibraryTests(unittest.TestCase):
    ...
    def test_section_is_visible_and_explains_solution_status(self):
        ...
        self.assertIn("Difficulté : {{ difficulty_description }}", PAGE)
        self.assertIn('{% assign difficulty_short = "N1" %}', PAGE)
        self.assertIn('{% assign difficulty_short = "N2" %}', PAGE)
        self.assertIn('{% assign difficulty_short = "N3" %}', PAGE)
        self.assertIn("N1 · Essentiel", PAGE)
        self.assertIn("N2 · Examen", PAGE)
        self.assertIn("N3 · Approfondissement", PAGE)
        self.assertNotIn(">{{ exercise.difficulty }}</span>", PAGE)
    ...
    def test_interactive_table_of_contents_covers_and_tracks_exercises(self):
        self.assertIn("Mini table des matières", PAGE)
        ...
        self.assertIn("if (tocDetails) tocDetails.open = false", SCRIPT)
        self.assertIn("event.key !== 'Escape'", SCRIPT)
        self.assertNotIn("tocDetails.open = true", SCRIPT)
        self.assertIn("position: sticky", STYLES)
        self.assertIn("top: max(0.5rem, env(safe-area-inset-top))", STYLES)
        self.assertIn(".mat101-toc-panel", STYLES)
        self.assertIn("position: absolute", STYLES)
        self.assertIn(".mat101-toc-level-advanced", STYLES)
